...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        axios.put("/user", { 'picture' :userData.picture},{
    ...
    return (
        <>
            ...
            {/* dialog  */}
            <Dialog label={label} open={modalOpen} validate={validate[0]} error={error} handleClose={async (value) => {
                try {
                    const token = localStorage.getItem("token")

                    await axios.put("/user", { [keyClicked]: value }, {
                        headers: {"Authorization" : `Bearer ${token}`}
                    });
